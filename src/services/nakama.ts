...
const resolvedHost = import.meta.env.VITE_NAKAMA_HOST || (isLocalNetwork ? window.location.hostname : PRODUCTION_NAKAMA_HOST);

const resolvedPort = import.meta.env.VITE_NAKAMA_PORT || (isLocalNetwork ? '7350' : PRODUCTION_NAKAMA_PORT);

const useSSL = import.meta.env.VITE_NAKAMA_SSL === 'true' || 
               import.meta.env.VITE_NAKAMA_USE_SSL === 'true' || 
               (!isLocalNetwork && isHttps);
