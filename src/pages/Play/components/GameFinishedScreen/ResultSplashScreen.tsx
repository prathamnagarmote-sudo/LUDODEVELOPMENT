...
function ResultSplashScreen({ text, playerColour }: Props) {
  ...
  const is2nd = text.includes('2nd');
  const is3rd = text.includes('3rd');
  ...
  if (is2nd || is3rd) {
    ...
    return (
      ...
      >
        ...
        <div className={`${styles.losePopupCard} ${styles.winPopupCard}`}>
          <div className={styles.winningTokenContainer} style={{ marginTop: '20px' }}>
            <TokenImage
              className={styles.winningToken}
              style={{
                '--fill-colour': woodStainColours[activeColour]
          ...
          <h1 className={styles.congratsText}>
            Congratulations!<br/>You secured {is2nd ? '2nd' : '3rd'} Position.
          </h1>
        </div>
      </motion.div>
    );
